...
public class RequestWrapper extends HttpServletRequestWrapper {
    ...
    public RequestWrapper(HttpServletRequest request, Integer o) {
        this(request);
        //接下来的request使用这个
...
//                    Arrays.stream(split).forEachOrdered(aesStr -> str.set(str + AESUtil.decryptCBC(aesStr)));
                    String str = AESUtil.decryptCBC(s);
                    ...
                    if (jsonArray != null){
                        ...
                        jsonArray.stream().forEach(json -> {
                            ...
                            Set<String> jsonSet = null;
                            try {
                                jsonSet =JSONUtil.parseObj(json).keySet();
                            }catch (Exception e){
                                jsonArray1.add(json);
                            }
                            if (jsonSet != null){
                                JSONUtil.parseObj(json).keySet().stream().forEach(time -> {
                                    if (time.indexOf("time") != -1 || time.indexOf("Time") != -1) {
                                        if (StringUtils.hasText(str) && str.indexOf("-") == -1){
                                            ...
                                            jsonObject1.put(time, localDateTime.format(formatter));
                                        }else
                        ...
                        if (x.indexOf("time") != -1 || x.indexOf("Time") != -1) {
                            if (StringUtils.hasText(str) && str.indexOf("-") == -1){
                                Long aLong = Long.valueOf(str);
                                LocalDateTime localDateTime =
                                        LocalDateTime.ofEpochSecond(aLong / 1000, 0, ZoneOffset.ofHours(8));
                                DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
                                jsonObject.put(x, localDateTime.format(formatter));
                            }else {
